...
export default function ProjectCard({ project }: { project: Project }) {
  ...
  const suitColor =
    suit === "♥" || suit === "♦" ? "text-[#DC143C]" : "text-gray-900";
  ...
  return (
    ...
    >
      <div className="relative rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-200 bg-white border-3 border-gray-200 hover:border-[#DC143C]">
        <div
          className={`absolute top-2 left-2 z-10 ${suitColor} font-black text-xl bg-white/95 backdrop-blur-md px-2 py-1 rounded-lg shadow-lg`}
        >
          {suit}
        </div>
        ...
        <div className="p-3 bg-gradient-to-br from-white to-gray-50">
          <div className="flex items-center justify-between mb-2">
            ...
            {project.shipped && (
              <span
                className="bg-gradient-to-r from-emerald-500 to-green-600 text-white text-xs font-bold px-2 py-1 rounded-full shadow-md"
                aria-label="Project shipped"
              >
                Shipped
              </span>
            )}
        ...
        <div
          className={`absolute bottom-2 right-2 ${suitColor} font-black text-xl bg-white/95 backdrop-blur-md px-2 py-1 rounded-lg shadow-lg rotate-180`}
        >
          {suit}
        </div>
      </div>
